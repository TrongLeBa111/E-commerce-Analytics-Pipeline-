...
[dbt] mart_customer_features.sql ← feature engineering (16 features)
        │
        ▼
notebooks/Churn_prediction.py
        │
        ├── EDA: Class imbalance + feature correlation
        ├── Preprocessing: SimpleImputer + LabelEncoder
        ├── Baseline: Logistic Regression
        ├── Main: LightGBM + class_weight tuning
        ├── Threshold tuning (Precision-Recall)
        ├── Evaluation: ROC, PR Curve, SHAP
        └── Score toàn bộ 96K customers
        │
        ▼
marts.churn_scores               ← output: probability + risk segment
ml/models/churn_model.pkl        ← trained model artifact
```
...
**Feature bị loại bỏ:**
- `avg_purchase_gap_days` — toàn NaN với 97% one-time buyers, không có giá trị phân biệt
- `recency_days` — **data leakage** — tương quan trực tiếp với label (`days_since_last_order > 90 → churned`). Loại bỏ trước khi train để tránh AUC ảo
...
### EDA — Class Distribution & Feature Correlation
![Churn EDA](../notebooks/results/07_churn_eda.png)

### Model Evaluation — Confusion Matrix, ROC, PR Curve
![Churn Evaluation](../notebooks/results/08_churn_evaluation.png)

### Feature Importance (SHAP)
![SHAP Importance](../notebooks/results/09_shap_importance.png)
...
| `ml/models/churn_model.pkl` | Trained LightGBM model |
